...
public class AutoComplete implements QuerySetExtension {
				...
				if(realSize <= 0) {
					...
					if(qcQuery != null && !qcQuery.equals("")) {
						...
						if(originQuery.length()==1) {
							whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
							whereList.add(new WhereSet("IDX_KEYWORD_UG", Protocol.WhereSet.OP_HASALL, originQuery)); //유니그램
						}else if(originQuery.length()>=2) {
							whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
							whereList.add(new WhereSet("IDX_KEYWORD_BI", Protocol.WhereSet.OP_HASALL, originQuery)); //바이그램
						}
						whereList.add(new WhereSet(Protocol.WhereSet.OP_BRACE_CLOSE));
						whereList.add(new WhereSet(Protocol.WhereSet.OP_AND));
						whereList.add(new WhereSet("IDX_COLLECTION_ID", Protocol.WhereSet.OP_HASALL, schTerm)); //tc 구분값
	...
	private WhereSet[] whereSet_fn(String search, String schTerm) {
		...
		if(search.length()==1) {
			whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
			whereList.add(new WhereSet("IDX_KEYWORD_UG", Protocol.WhereSet.OP_HASALL, search)); //유니그램
		}else if(search.length()>=2) {
			whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
			whereList.add(new WhereSet("IDX_KEYWORD_BI", Protocol.WhereSet.OP_HASALL, search)); //바이그램
		}
		whereList.add(new WhereSet(Protocol.WhereSet.OP_BRACE_CLOSE));
		whereList.add(new WhereSet(Protocol.WhereSet.OP_AND));
		whereList.add(new WhereSet("IDX_COLLECTION_ID", Protocol.WhereSet.OP_HASALL, schTerm)); //tc 구분값
	...
	private SelectSet[] selectSet_fn() {
		SelectSet[] selectSet = new SelectSet[] {
				...
				new SelectSet("KEYWORD2", (byte) (Protocol.SelectSet.NONE))
