...
public class EventStrategy implements QuerySetExtension, ResultJsonExtension {
...
//			}
	...
	private Query query_fn(Map<String, String> SearchUtil, int queryCheck) throws Exception {
		...
		String strtgyCtgry = SearchUtil.get("strtgyCtgry") != null ? SearchUtil.get("strtgyCtgry") : "";  // 이벤트 전략 카테고리
		...
		SearchUtil.put("strtgyCtgry", strtgyCtgry); // 추가 10.30
...
//				query.setFrom("STG_MAIN");
	...
	private WhereSet[] whereSet_fn(Map<String, String> SearchUtil, int queryCheck) {
		...
		if(queryCheck==1) { // 상품검색일때
			// 카테고리코드 시작
			if (!SearchUtil.get("category01").equals("") || !SearchUtil.get("category02").equals("")|| !SearchUtil.get("category03").equals("") 
														|| !SearchUtil.get("category04").equals("")|| !SearchUtil.get("category05").equals("")) {
			...
			//이벤트 전략 카테고리 검색
			if (!SearchUtil.get("strtgyCtgry").equals("")) {
				if (whereList.size() > 0) whereList.add(new WhereSet(Protocol.WhereSet.OP_AND));
				whereList.add(new WhereSet(Protocol.WhereSet.OP_BRACE_OPEN));
				whereList.add(new WhereSet("IDX_STRTGY_CTGRY1_WS", Protocol.WhereSet.OP_HASALL,SearchUtil.get("strtgyCtgry"), 1));
				whereList.add(new WhereSet( Protocol.WhereSet.OP_OR));
				whereList.add(new WhereSet("IDX_STRTGY_CTGRY2_WS", Protocol.WhereSet.OP_HASALL,SearchUtil.get("strtgyCtgry"), 1));
				whereList.add(new WhereSet( Protocol.WhereSet.OP_OR));
				whereList.add(new WhereSet("IDX_STRTGY_CTGRY3_WS", Protocol.WhereSet.OP_HASALL,SearchUtil.get("strtgyCtgry"), 1));
				whereList.add(new WhereSet( Protocol.WhereSet.OP_OR));
				whereList.add(new WhereSet("IDX_STRTGY_CTGRY4_WS", Protocol.WhereSet.OP_HASALL,SearchUtil.get("strtgyCtgry"), 1));
				whereList.add(new WhereSet(Protocol.WhereSet.OP_BRACE_CLOSE));
			}
	...
	private SelectSet[] selectSet_fn(Map<String, String> SearchUtil, int queryCheck) {
		...
		if (queryCheck == 1) {
			selectSet = new SelectSet[] { 
					...
					/*추가 10.30*/
					new SelectSet("STRTGY_CTGRY1", (byte) (Protocol.SelectSet.NONE)),
					new SelectSet("STRTGY_CTGRY1_NM", (byte) (Protocol.SelectSet.NONE)),
					new SelectSet("STRTGY_CTGRY2", (byte) (Protocol.SelectSet.NONE)),
					new SelectSet("STRTGY_CTGRY2_NM", (byte) (Protocol.SelectSet.NONE)),
					new SelectSet("STRTGY_CTGRY3", (byte) (Protocol.SelectSet.NONE)),
					new SelectSet("STRTGY_CTGRY3_NM", (byte) (Protocol.SelectSet.NONE)),
					/*추가 11.07*/
					new SelectSet("STRTGY_CTGRY4", (byte) (Protocol.SelectSet.NONE)),
					new SelectSet("STRTGY_CTGRY4_NM", (byte) (Protocol.SelectSet.NONE))};
		}
