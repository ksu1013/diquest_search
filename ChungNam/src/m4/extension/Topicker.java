...
public class Topicker implements DbWatcherExtension {
...
//	private String jianaHome = "/home/chungnam/mariner4/resources/korean";
	...
	private StringBuffer EPOTION_TYPE		= new StringBuffer();
	private StringBuffer KEYWORD_PERSON		= new StringBuffer();
	private StringBuffer KEYWORD_ORGAN		= new StringBuffer();
	...
	public void start(DbWatcher watcher) throws SQLException {
		...
		String collectionId = "CHUNGNAM_NEWS";
		try {
...
//		if(disa != null) disa.fine();
	...
	public class exTractorTopicker extends FieldMapper {
		...
		public String mapping(DbColumnValue[] columnValue) throws SQLException { 
	    	...
	    	if(!currentId.equals(hasId)) {
	    		INITIAL_KEYWORD = columnValue[getIndexOf("CONTS_CONTS", columnValue)].getString();// DQ_DOC의 ID값에 해당하는 value값.
	    		INITIAL_KEYWORD2 = columnValue[getIndexOf("CONTS_TITLE", columnValue)].getString();// DQ_DOC의 ID값에 해당하는 value값.
	    		data_fkey = new StringBuilder();// 최종적인 결과물을 담아내서 return 하기위한 최종 결과물역할 변수.
	    		String kkkk="";
	    		kkkk = exTractorTopicker(INITIAL_KEYWORD);
	    		if(!kkkk.equals("")) {
	    			kkkk= kkkk+";"+exTractorTopicker(INITIAL_KEYWORD2);
	    		}
	    		else {
	    			kkkk = exTractorTopicker(INITIAL_KEYWORD2);
	    		}
	    		...
	    		EPOTION_TYPE.delete(0, EPOTION_TYPE.length());
	    		KEYWORD_PERSON.delete(0, KEYWORD_PERSON.length());
	    		KEYWORD_ORGAN.delete(0, KEYWORD_ORGAN.length());
	    		DISA_INPUT.delete(0,  DISA_INPUT.length());

	    		DISA_INPUT.append(getValueByFieldName("CONTS_TITLE", columnValue) + " " + getValueByFieldName("CONTS_CONTS", columnValue));
	    	...
	    	if(fieldName.equals("KEYWORD_PERSON"))				return KEYWORD_PERSON.toString();
	    	if(fieldName.equals("KEYWORD_ORGAN"))				return KEYWORD_ORGAN.toString();
			...
			for(int i = 0; i < dsr.getEntitySize(); i++){
				...
				if(type!=null && !type.equals("")) {
					if(type.equals("$EMOTION_NEG")) {
						...
						String postStr = cottonExtract(key, cotton, cb);
					...
					if(type.equals("$CHUNGNAM_ORGANIZATION")) {
						if(KEYWORD_ORGAN.length() > 0) {
							splitUnit = ",";
						}
						KEYWORD_ORGAN.append(splitUnit + key);
					}
					if(type.equals("$CHUNGNAM_PERSON")) {
						if(KEYWORD_PERSON.length() > 0) {
							splitUnit = ",";
						}
						KEYWORD_PERSON.append(splitUnit + key);
					}
				}
	...
	/**
	 * 명사추출기 수준의 분석 및 결과출력 sample
	 * @param inputStr
	 * @return
	 */
	public static String cottonExtract(String inputStr, JianaCotton cotton, CottonBuffer cb){
		String jianaStr = "";
